...
func setJobLabels(m *v1.ObjectMeta, j *jobs.Job) {
	setLabel(m, k8s.LabelJobName, k8s.ResourceName(j.Name.String()))
}
...
func setTaskGroupLabelsAnnotations(m *v1.ObjectMeta, tg *jobs.TaskGroup) {
	...
	setLabel(m, k8s.LabelTaskGroupName, k8s.ResourceName(tg.Name.String()))
	setLabel(m, k8s.LabelTaskGroupFullName, k8s.ResourceName(tg.FullName()))
}
...
func setPodLabels(m *v1.ObjectMeta, tg *jobs.TaskGroup, pod pod) {
	...
	setLabel(m, k8s.LabelPodName, k8s.ResourceName(pod.name))
}
...
func createPodSelector(input map[string]string, pod pod) map[string]string {
	...
	input[k8s.LabelPodName] = k8s.ResourceName(pod.name)
	return input
}
